...
<?php
if(($room=='Room name')|($room=='')){}elseif(preg_match("/^[a-z0-9]{3,}/",$_POST['room'])){
	$room=preg_replace('/[^a-z0-9]/i','',$room);
	$room=preg_replace('/\s+/','',$room);
	$data=file_get_contents('http://tinychat.apigee.com/'.strtolower($room).'.xml');
