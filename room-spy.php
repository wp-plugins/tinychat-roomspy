...
function wp_show_room_spy() {
	...
	?>
    <style>#chat,.chatimages{margin:2px 6px 15px;width:220px;height:200px;-webkit-transition:all .3s ease;-moz-transition:all .3s ease;-o-transition:all .3s ease;-ms-transition:all .3s ease;transition:all .3s ease;display:inline-block;text-decoration:none;font-size:17px;-webkit-border-radius:10px;-moz-border-radius:10px;border-radius:4px}input[type=text]{width:25%;}.entry-content img,img[class*=wp-image-]{height:190px;}.password{color:red;text-decoration:none}</style>
<?php
	$room = $_POST ['room']; 
...
<form method="post"><input type="text" name="room" title="Just enter the name of the tinychat room and press spy." placeholder="Room Name"/> <input type="submit" value="Spy"/></form> </div><?php

if((preg_match("/^[a-zA-Z0-9]{3,}/",$_POST['room'])=='1')){$room=preg_replace('/[^a-zA-Z0-9]/s','',$room);$room=strtolower($room);$room=htmlspecialchars($room,ENT_QUOTES, 'UTF-8');$room=strip_tags($room);if($room!='room'){echo '<br />Statistics: &nbsp;&bull;&nbsp;Admins: '.$rooms['mod_count'].'&nbsp;&bull;&nbsp;Chatters: '.$rooms['total_count'].'&nbsp;&bull;&nbsp;On Cam: '.$rooms['broadcaster_count'].'<a class="password" title="Password is required to enter the room"/>'.$rooms['error'].'</a>';if($room!=='room')echo '<br><br>';{echo ' <br> '.$room.'<br>';$pic='//upload.tinychat.com/pic/'.$room.'';$picture='<a href="//www.ruddernation.net/'.urlencode($room).'" title="'.$room.'" target="_blank"><img src="'.$pic.'"class="chatimages"><br></a>';echo $picture;}echo '<br><br>';foreach($rooms->names as $username){echo '<div id="chat">'.$username.'<br>';$pic='//upload.tinychat.com/i/'.$room.'-'.$username.'.jpg';$picture='<a href="//www.ruddernation.net/'.$room.'" title="Hey! I&#39;m '.$username.', Click to come and chat with me." target="_blank"><img src="'.$pic.'"class="chatimages"></a><br></div>';echo $picture;}}}}?>